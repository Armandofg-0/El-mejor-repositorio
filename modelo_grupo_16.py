...
data = {}
data["delta_q"] = pd.read_csv('cuadrantes.csv', usecols=['demanda_por_dia']).squeeze().values # Matriz que contiene: la demanda de cada cuadrante por horas (posible cambio a días)
data['upsilon_c'] = pd.read_csv('comisarias.csv', usecols=['total_patrullas_por_comisaria']).squeeze().values
...
data['rho_c'] = pd.read_csv('comisarias.csv', usecols=['presupuesto_diario_comisaria']).squeeze().values
data['theta_{q,h}'] = pd.read_csv('crimenes_por_hora.csv', header=None)
...
data['B'] = pd.read_csv('comisarias.csv').squeeze().values           
...
R2 = m.addConstrs(
    (quicksum(p[c, t] for t in T) <= data['upsilon_c'] for c in C),
    name="R2"
)
